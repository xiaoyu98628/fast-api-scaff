...
async def test_regular_request_returns_driver_independent_buffered_response() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, headers=[("set-cookie", "a=1"), ("set-cookie", "b=2")], json={"ok": True})
...
@pytest.mark.asyncio
async def test_httpx_timeout_is_translated() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
...
def _resource(handler: Callable[[httpx.Request], Coroutine[None, None, httpx.Response]]) -> HttpxResource:
    transport = httpx.MockTransport(handler)
    return HttpxResource(
        standard_client=httpx.AsyncClient(transport=transport),
        stream_client=httpx.AsyncClient(transport=transport),
    )
