...
登录 Lua 脚本归属用户上下文，HTTP 固定窗口脚本归属独立限流组件；公共 Redis 层提供统一 key 前缀、TTL 与 `execute_script`，不再提供 `increment`、`delete_below`、`acquire_window` 场景方法。缓存契约分别位于 `contracts/storage.py`、`contracts/redis.py` 和 `contracts/script.py`，详见[缓存分层](docs/cache.md#2-组件分层)。扩展原则见[架构说明](docs/architecture.md#redis-场景扩展边界)。
