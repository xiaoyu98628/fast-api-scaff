...
针对性命令：`uv run python -m pytest -q tests/queue tests/worker`。测试覆盖实际内存队列与 SQLite 失败存储，外部驱动使用模拟客户端；不等同于真实服务验证。tests 根目录是空初始化文件的 Python 包，避免 tests/queue 被 pytest 当作标准库 queue 同名顶级包。
