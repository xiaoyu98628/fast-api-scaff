...
脚手架内置 `LoginSucceededJob` 和 `CleanupExpiredSessionsJob`。登录接口向默认连接配置的默认队列（`sample.env` 为 `default`）尽力投递前者；Scheduler 在容器本地时间每天 0 点向同一默认队列投递后者。登录任务通过用户应用服务查询最新用户并记录不含认证秘密的固定事件；清理任务通过认证应用服务在独立事务中批量删除过期会话，并记录删除数量。两个任务都只把连接池超时、断连和驱动明确标记的失效连接转换为可重试错误，其他数据库错误直接进入失败存储。`sample.env` 以 Redis 为默认连接，因此 Worker 可以不带参数启动。
