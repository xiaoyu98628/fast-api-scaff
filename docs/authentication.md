...
登录成功并提交会话后，HTTP 适配器通过后台任务向默认连接配置的默认逻辑队列（`sample.env` 为 `default`）投递 `LoginSucceededJob`。消息以 `user_id` 参数标识登录用户，并包含固定文案“用户登录成功，队列任务已执行。”，不包含用户名、密码或 Token；当前 HTTP request ID 会显式写入消息的 `correlation_id`。独立 Worker 消费后使用当前 `JobExecutionContext` 调用任务的 `handle(context)`，记录该文案和结构化用户 ID，并使任务调用链日志能够与原请求关联：
...
- 用户不存在时直接返回 404 和“用户不存在”，不执行密码验证、不创建会话。密码验证后重新读取时用户已被删除，也返回 404。
- 用户名不符合领域格式规则、错误密码或禁用账户返回 401 和“用户名或密码错误”。用户名格式无效时不执行密码验证。
...
- 登录用户不存在的 404 同样带 `Cache-Control: no-store`。此示例明确区分账户不存在，不隐藏账户是否存在。
...
登录慢哈希在事务外执行，随后重新核对用户、密码哈希和状态再保存会话。这不提供与并发密码重置严格串行的保证；当前方案没有认证版本、数据版本或改密全设备退出能力。

创建用户会先校验用户名、邮箱和密码，并在 Argon2 哈希前执行用户名、邮箱占用预检查；哈希后仍会重新检查并依赖数据库唯一约束处理并发竞争。密码重置先确认目标用户存在，哈希完成后再重新读取并更新，因此重复创建和不存在用户的重置请求不会占用 Argon2 额度。
