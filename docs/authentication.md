...
每次登录建立独立会话，并在写入新会话的同一事务中删除全局已过期会话；退出只删除当前会话。内置 `users.sessions.cleanup_expired` 计划还会在容器本地时间每天 0 点向默认队列投递 `CleanupExpiredSessionsJob`，Worker 通过认证应用服务在独立事务中删除到当前时间已经过期的会话。请求认证时读取当前用户状态，因此：
...
密码重置、用户禁用及其会话撤销使用同一个 `UserUnitOfWork` 和数据库事务，任一步骤失败都会整体回滚；启用账户不创建或恢复会话。外键声明删除级联；SQLite Provider 会为每个新连接启用外键约束，因此删除用户会同步删除其会话。过期会话由后续成功登录和每日清理任务共同回收；数据库连接池超时、断连或驱动明确标记的失效连接会进入 Worker 投递内重试，其他数据库错误进入失败存储。任务使用条件删除，重复消费不会删除仍然有效的会话。
...
本示例不提供角色权限、IP 登录限流、客户端 Token 存储策略、自助改密或刷新令牌。现有公开用户 CRUD 不能因为增加了登录示例就被视为受保护接口。
